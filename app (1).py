...
def load_model():
    ...
    ann    = joblib.load(os.path.join(base, 'models', 'ann_model.pkl'))
...
def predict_student(data_dict):
    """Build a feature vector from the input dict and run the ANN."""
    row = {}
    cat_defaults = {
...
def show_sidebar():
    with st.sidebar:
        ...
        st.markdown(f"**User:** {st.session_state.registered_users[st.session_state.username]['name']}")
...
def page_dashboard():
    ...
    with col_b:
        ...
        try:
            img_path = os.path.join(os.path.dirname(__file__), 'assets', 'loss_curve.png')
            if os.path.exists(img_path):
                st.image(img_path, use_container_width=True)
        except Exception:
            st.info("Run the notebook first to generate plots.")

    st.subheader("📖 Quick Start Guide")
    role = st.session_state.user_role
    if role == 'student':
        st.info("👉 Go to **My Prediction** to enter your details and get a performance forecast.")
    elif role == 'teacher':
        st.info("👉 Go to **Predict Student** to assess individual students, or **Batch Upload** for a full class CSV.")
    else:
        st.info("👉 Full access: predict students, upload batches, view analytics, and manage users.")


def student_input_form(key_prefix=""):
    """Reusable student data entry form. Returns dict of inputs."""
    ...
    with c2:
        ...
        Mjob    = st.selectbox("Mother Job", ["teacher","health","services","at_home","other"], key=f"{key_prefix}_Mjob")
        Fjob    = st.selectbox("Father Job", ["teacher","health","services","at_home","other"], key=f"{key_prefix}_Fjob")
...
def show_prediction_result(pred, proba, student_name="Student"):
    """Display a styled prediction result."""
    st.markdown("---")
    ...
    with col_g:
        ...
        for i in range(len(colors)):
            ax.barh(0, boundaries[i+1]-boundaries[i], left=boundaries[i],
                    height=0.4, color=colors[i], alpha=0.7)
        ax.axvline(proba, color='#1e293b', lw=3, label=f'Score: {proba:.1%}')
        ax.set_xlim(0,1); ax.set_ylim(-0.5,0.8)
        ax.set_xticks([0,0.4,0.7,1.0]); ax.set_xticklabels(['0%','40%','70%','100%'])
...
def page_predict(title="🔮 Predict Student Performance"):
    ...
    if st.button("🔮 Run ANN Prediction", use_container_width=True):
        ...
        st.success(f"Prediction saved to history ({len(st.session_state.history)} total).")
...
def page_batch():
    ...
    st.markdown("""
    Upload a CSV file with student data. Required columns include:
...
def page_history():
    ...
    if 'prediction' in df.columns:
        col2.metric("Pass Rate", f"{(df['prediction']=='Pass').mean():.1%}")

    # Download
    csv = df.to_csv(index=False).encode()
    st.download_button("⬇️ Download History", csv, "history.csv", "text/csv")
...
def page_analytics():
    ...
    st.info("Analytics are generated from the training dataset.")
    ...
    for fname, title in plots:
        ...
        if os.path.exists(path):
            ...
            st.warning(f"{fname} not found — run the Jupyter notebook first.")
...
def page_manage_users():
    ...
    users = st.session_state.registered_users
    data = [{'Username': u, 'Name': v['name'], 'Role': v['role']}
...
def page_student_prediction():
    """Student-specific prediction page with fewer fields visible."""
...
def page_student_history():
    ...
    username = st.session_state.username
    hist = [h for h in st.session_state.history if h.get('by') == username]
    if not hist:
